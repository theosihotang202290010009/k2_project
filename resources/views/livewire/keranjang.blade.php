                ...
                <table class="table text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>
                                <img src="{{ url('assets/produk') }}/{{ $keranjang->produk->gambar }}" class="img-fluid" width="200">
                            </td>
                            <td>
                                {{ $keranjang->produk->nama_pdk }}
                            </td>
                            <td>{{ $keranjang->jumlah_pesanan }}</td>
                            <td>Rp. {{ number_format($keranjang->product->harga) }}</td>
                            <td><strong>Rp. {{ number_format($keranjang->total_harga) }}</strong></td>
                            <td>
                                <i wire:click="destroy({{ $keranjang->id }})" class="fas fa-trash text-danger"></i>
                            </td>
                        </tr>    
                        @empty
                        <tr>
